...
import {
  ...
  getSheetsMaps,
} from '@/lib/features/labels/excel';

interface CheckListUploadProps {
  // Notify parent about available sheets and their raw JSON rows
  onSheetsDetected?: (sheets: Record<string, any[]>) => void; // eslint-disable-line @typescript-eslint/no-explicit-any
  // Notify parent about formatted JSON rows (with cell formats like % and € applied)
  onSheetsDetectedFormatted?: (sheets: Record<string, any[]>) => void; // eslint-disable-line @typescript-eslint/no-explicit-any
  // Parent indicates that a file has been uploaded (sheets detected)
  fileUploaded?: boolean;
  // Notify parent when user selects a new file (so parent can re-enable upload)
  onNewFileSelected?: () => void;
}

export interface CheckListUploadRef {
  clearFile: () => void;
}

const CheckListUpload = forwardRef<CheckListUploadRef, CheckListUploadProps>(
  (
    {
      onSheetsDetected,
      onSheetsDetectedFormatted,
      fileUploaded,
      onNewFileSelected,
    },
    ref
    ...
    const handleUpload = () => {
      ...
      if (!isSupportedExcelFile(selectedFile.name)) {
        setError(
          'Bitte wählen Sie eine gültige Excel-Datei (.xlsx oder .xls).'
        );
        setIsLoading(false);
        return;
      }
      ...
      reader.onload = (e) => {
        try {
          ...
          const { raw: sheetsMapRaw, formatted: sheetsMapFmt } =
            getSheetsMaps(workbook);
          ...
          setError(
            'Fehler beim Verarbeiten der Datei. Bitte überprüfen Sie die Dateistruktur.'
          );
        } finally {
      ...
      <div className='space-y-4'>
        ...
        {selectedFile && (
          <p className='text-xl font-bold text-black'>
            Ausgewählte Datei:{' '}
            <span className='text-black text-lg'>{selectedFile.name}</span>
          </p>
        )}
      </div>
    );
...
CheckListUpload.displayName = 'CheckListUpload';
