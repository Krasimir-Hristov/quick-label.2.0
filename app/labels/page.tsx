...
import FileUpload, { FileUploadRef } from '@/components/labels/FileUpload';
import LabelPreview from '@/components/labels/LabelPreview';
import LabelSheetForPrint from '@/components/labels/LabelSheetForPrint';
import { LabelData, ProcessedProduct } from '@/types';
import { Carrot, Rabbit } from 'lucide-react';
import { processProducts } from '@/lib/features/labels/productProcessor';
        ...
        if (sheetsMapFormatted && sheetName in sheetsMapFormatted) {
          ...
          const source: ProcessedProduct[] =
            (selectedRegion === 'GermanyD1'
              ? result.germanyD1Products
              : selectedRegion === 'GermanyD2'
              ? result.germanyD2Products
              : selectedRegion === 'Austria'
              ? result.austriaProducts
              : result.beneluxProducts) ?? [];
          const limitedSource =
            currentUser === 'ZOO' ? source.slice(0, 10) : source;
          const mapped: LabelData[] = limitedSource.map(
            (p: ProcessedProduct) => ({
